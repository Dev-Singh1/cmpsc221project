...
public class MultiTableQueries {
    ...
    public static ArrayList<ClassDescription> getAllClassDescriptions(String currentSemester) {
        ...
        try
        {
            getDesc = connection.prepareStatement("select app.class.coursecode,app.course.description,app.class.seats from app.class,app.course");
    ...
    public static ArrayList<StudentEntry> getScheduledStudentsByClass(String semester, String courseCode){
       ...
       try
       {
           getScheduledStudents=connection.prepareStatement("select app.student.firstname,app.student.lastname,app.student.id from app.student inner join app.schedule on app.student.id=app.schedule.studentid where app.schedule.coursecode like (?) and app.schedule.semester like (?) and app.schedule.status like (?) order by app.schedule.timestamp");
    ...
    public static ArrayList<StudentEntry> getWaitlistedStudentsByClass(String semester, String courseCode){
       ...
       try
       {
           getWaitStudents=connection.prepareStatement("select app.student.firstname,app.student.lastname,app.student.id from app.student inner join app.schedule on app.student.id=app.schedule.studentid and app.schedule.coursecode like (?) and app.schedule.semester like (?) and app.schedule.status like (?) order by app.schedule.timestamp");
